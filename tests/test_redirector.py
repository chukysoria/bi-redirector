...
def test_authenticate(webapp, box_redis_store, oauth, redisdb):
    ...
    oauth.get_authorization_url.assert_called_once_with(f'https://{HEROKU_APP_NAME}.herokuapp.com/boxauth')
...
def test_box_auth(webapp, box_redis_store, redisdb, oauth):
    ...
    assert response.status_code == 200
    assert response.data.decode() == 'Authenticated. You can close this window.'
